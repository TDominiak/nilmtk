...
class Seq2Seq(Disaggregator):
        ...
        for test_mains_df in test_main_list:
            ...
            for appliance in self.models:
                ...
                if not self.appliance_params:
                    ...
                    normalization_file_path = f'normalization_{appliance}.json'
                    with open(normalization_file_path) as f:
                        self.appliance_params = json.load(f)
    ...
    def set_appliance_params(self,train_appliances):
        ...
        for (app_name, df_list) in train_appliances:
            ...
            normalization_file_path = f'normalization_{app_name}.json'
            self.appliance_params.update({app_name: {'mean':app_mean, 'std':app_std}})

            with open(normalization_file_path, 'w') as file:
                file.write(json.dumps(self.appliance_params, indent=4))

    def load_model(self, model_path):
        print('Loading models...')
        for appliance in self.load_model_path.keys():
            model = self.return_network()
            model.load_weights(model_path[appliance])
